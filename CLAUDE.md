...
2. [프로젝트 구조](#프로젝트-구조)
3. [서버 및 인프라 정보](#서버-및-인프라-정보)
4. [시스템 아키텍처](#시스템-아키텍처)
5. [CI/CD 자동 배포](#cicd-자동-배포)
6. [상세 배포 가이드](#상세-배포-가이드)
7. [전략 생성 가이드](#전략-생성-가이드)
8. [AI 에이전트 아키텍처](#ai-에이전트-아키텍처)
9. [핵심 데이터 구조](#핵심-데이터-구조)
10. [절대 하면 안 되는 것들](#절대-하면-안-되는-것들)
11. [파일별 수정 규칙](#파일별-수정-규칙)
...
> **⚠️ CRITICAL**: 서버에는 여러 프로젝트가 공존합니다. 배포/재시작 시 **반드시** Group C만 영향받도록 해야 합니다.

### 서버 프로젝트 구조

```
Seoul Server (141.164.55.245)
├── /root/proxy/       → Global Proxy (Nginx) - 공유 인프라
├── /root/group_a/     → Stock Trading AI
├── /root/group_b/     → Automation
├── /root/group_c/     → ⭐ AI Trading Platform (이 프로젝트)
└── /root/group_e/     → N8N Automation
```
...
| 항목 | 분리 방법 |
|------|----------|
| **컨테이너 이름** | `groupc-` 접두사 사용 (groupc-backend, groupc-frontend 등) |
| **네트워크** | `group_c_network` 내부 통신, `proxy-net`은 외부 접근용 공유 |
| **포트** | 3200-3299 범위 전용 사용 |
| **볼륨** | `groupc_` 접두사 사용 (groupc_postgres_data 등) |
| **배포 스크립트** | `deploy.sh group_c deploy` - Group C만 영향 |

### 안전한 배포 명령어

```bash
# ✅ 안전: Group C만 배포 (다른 그룹 영향 없음)
./deploy.sh group_c deploy

# ✅ 안전: Group C만 재시작
./deploy.sh group_c restart
...
```bash
# ❌ 위험: 모든 컨테이너에 영향
docker compose down  # 프로젝트 경로 없이 실행
...
- **Group C 전용**: `/root/group_c` 경로만 사용
- **안전한 재시작**: `docker compose up -d --force-recreate` 사용 (down 없음)
- 다른 그룹에 절대 영향 없음
